# AI Vibe Coding - Full Stack JavaScript Techdegree Project 11

## Created By

- Name: Kendra Williams
- Date Completed: 1/2/26

---

## Tools Used

_List all the AI tools you utilized to complete the project._

- _(AI Tool One)_ ChatGPT
- _(AI Tool Two)_ Copilot
- _(AI Tool Three)_

---

## What Worked

_Describe what parts of the project went smoothly or turned out well. This could include specific features, code organization, approaches to prompts or workflow habits._

- _(What Worked 1)_ Code organization
- _(What Worked 2)_ Creating code dynamically
- _(What Worked 3)_ 

---

## What Didn't Work

_Explain what parts of the project were challenging or didn't go as planned. Did you hit bugs, confusion, or dead ends?_

- _(What Didn't Work 1)_ There was a few bugs when dealing with the routes
- _(What Didn't Work 2)_ Sequelize errors
- _(What Didn't Work 3)_

---

## Pros / Cons of Using an AI Coding Assistant

### Pros

_What were the benefits of using an AI assistant during this project?_

- _(Benefit 1)_ Helped with keeping code organized and aligned
- _(Benefit 2)_ Helped with breaking down the errors and why it was caused
- _(Benefit 3)_

### Cons

_What were the downsides, limitations, or challenges when relying on AI during this project?_

- _(Downside 1)_ I did not have any downsides or limitations that prevented me from completing this project with AI assistance
- _(Downside 2)_
- _(Downside 3)_
